...
func (s *Server) Listen(ctx context.Context) {
	...
	if conn, err := net.ListenUDP("udp", s.BroadcastAddr); err != nil {
		...
		if err := conn.SetReadBuffer(types.MAX_MPDU); err != nil && s.rcvErrors {
			s.error <- err
		}
	}
...
func (s *Server) receive(conn *net.UDPConn, ctx context.Context) {
	...
	b := make([]byte, types.MAX_MPDU, types.MAX_MPDU)
...
func (s *Server) getConfirmedHandler(invokeId uint8) chan<- *Response {
	if invokeId == 0 {
...
func (s *Server) SetConfirmedHandler(invokeId uint8, handler chan<- *Response) {
	if invokeId == 0 {
...
func (s *Server) SetUnconfirmedHandler(service types.UnconfirmedService, handler chan<- *Response) {
